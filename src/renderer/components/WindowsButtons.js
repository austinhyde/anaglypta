...
const svgUrl = svg => svg.startsWith('data:image')? `url("${svg}")` : `url("data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg.trim())}")`
...
export default function WindowsButtons({
  ...
  const buttonStyle = css(styles.button, {
    ':hover': { backgroundColor: Color(color).alpha(0.2).rgbaString() },
    ':active': { backgroundColor: Color(color).alpha(0.3).rgbaString() },
    '& div': { backgroundColor: color },
  });

  return (
    <div {...styles.buttons} {...rest}>
      {customButtons.map(({ icon, iconStyle, iconProps, ...props }, i) =>
        <div key={i} {...css(buttonStyle, iconCss(icon, iconStyle))} {...props}><div {...iconProps}/></div>
      )}
